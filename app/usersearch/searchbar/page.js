...
export default function SearchBar ({users,myinfo}){
    ...
    return(
        ...
        <img src="https://hyunstagram.s3.ap-northeast-2.amazonaws.com/hyunstagram/search.png" alt="" style={{width:'20px'}}/>
            ...
            onFocus={(e)=>{console.log(e.currentTarget.value,'최근검색텍스트');  if(e.currentTarget.value){setOpenCurrent(false)}else{
                setOpenCurrent(true)}
            }}
        ...
        }
        {
            ...
            <div style={{display:'flex',flexDirection:'column', padding : '0 50px'}}>
            {
                ...
                <Link href={"/profile/" + a.name} key={i} style={{display:'flex', alignItems:'center', margin: '10px 0'}} >
                    <img src={'https://hyunstagram.s3.ap-northeast-2.amazonaws.com/hyunstagram/' + a.image} style={{width:'30px',borderRadius:'50%',overflow:'hidden', marginRight: '10px'}}/> <span dangerouslySetInnerHTML={{ __html: highlightText(a.name, text) }}></span>
                </Link>
                )
            }
        </div>
            ...
            <div style={{padding: '0 50px', display:'flex',flexDirection:'column'}}>
                <p style={{margin : '30px 0'}}>'{result}'에 대한 검색결과</p>
                {
                     users.filter((e)=> e.name.includes(result)).map((a,i)=>
                        <Link href={"/profile/" + a.name} key={i} style={{display:'flex', alignItems:'center', margin: '10px 0'}}>
                            <img src={'https://hyunstagram.s3.ap-northeast-2.amazonaws.com/hyunstagram/' + a.image} style={{width:'30px', marginRight: '10px',borderRadius:'50%',overflow:'hidden'}}/> <span>{a.name}</span>
                        </Link>
                        )
                }
                {
